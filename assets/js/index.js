...
$(document).ready(function () {
    $("nav ul li a").click(function () {
        $(".isikonten").slideToggle("fast");

        $('nav ul li.off').each(function (index, el) {
            var isikontenID = el.id.split('-')[1];
            $('.isikonten#' + isikontenID).hide();
        });
    });

    $('nav').delegate('.off', 'click', function (event) {
        var pilihisikonten = this.id.split('-')[1];
        console.log(pilihisikonten);
        document.location.hash = pilihisikonten;

        $('.isikonten:visible').fadeOut('slow', function () {
            $('.isikonten#' + pilihisikonten).fadeIn('slow');
        });
